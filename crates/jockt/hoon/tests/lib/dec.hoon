...
++  test-jeam
  %+  expect-eq:test
    !>  ^-  jock:jock
        [%func pos=[0 0] type=[p=[%core p=[%.y p=[inp=[~ [p=[%atom p=%number q=%.n] name='a']] out=[p=[%atom p=%number q=%.n] name='']]] q=~] name='dec'] body=[%lambda p=[arg=[inp=[~ [p=[%atom p=%number q=%.n] name='a']] out=[p=[%atom p=%number q=%.n] name='']] body=[%let pos=[0 0] type=[p=[%none p=~] name='b'] val=[%atom p=[[%number p=0] q=%.n]] next=[%loop next=[%if pos=[0 0] cond=[%compare comp=%'==' a=[%limb p=~[[%name p=%a]]] b=[%increment val=[%limb p=~[[%name p=%b]]]]] then=[%limb p=~[[%name p=%b]]] after=[%else then=[%edit limb=~[[%name p=%b]] val=[%increment val=[%limb p=~[[%name p=%b]]]] next=[%call pos=[0 0] func=[%limb p=~[[%axis p=0]]] arg=~]]]]]] context=~]] next=[%call pos=[0 0] func=[%limb p=~[[%name p=%dec]]] arg=[~ [%atom p=[[%number p=43] q=%.n]]]]]
    !>  (jeam:jock text)
::
++  test-mint
  %+  expect-eq:test
